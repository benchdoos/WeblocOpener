...
class SunManager {
    ...
    private TimeRange getDarkModeValueFromJson(JsonObject root) {
        ...
        final DateFormat timeFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");

        try {
            final Date sunriseDate = timeFormat.parse(sunriseTime);
            final Date sunsetDate = timeFormat.parse(sunsetTime);
